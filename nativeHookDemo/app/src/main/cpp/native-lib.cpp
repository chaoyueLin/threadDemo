...
#define HACKER_JNI_CLASS_NAME "com/example/nativehookdemo/ThreadHook"
...
static JavaVM *kJvm;
static pthread_p pthread_p_prev = NULL;
static bytehook_stub_t pthread_p_stub = NULL;
...
static int myPthread_create_proxy(pthread_t *thread, const pthread_attr_t *attr,
    ...
    LOG("hook_tag proxy thread create");
...
static int hacker_hook_thread(JNIEnv *env, jclass thiz) {
    (void) env, (void) thiz;
    pthread_p_stub = bytehook_hook_single("libart.so", NULL, "pthread_create",
                                          (void *) myPthread_create_proxy,
                                          pthread_p_hooked_callback, NULL);
    return 0;
}
